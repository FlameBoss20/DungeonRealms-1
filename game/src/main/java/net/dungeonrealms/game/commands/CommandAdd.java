...
public class CommandAdd extends BasicCommand {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String string, String[] args) {
        ...
        if (!Rank.isGM(player) && !DungeonRealms.getInstance().isBetaShard) {
                    return false;
        }
...
//                    RealmInstance.getInstance().openRealmPortal(player);
                ...
                case "weapon":
                    try {
                        if (args.length == 2) {
                            ...
                            ItemStack itemStack = new ItemGenerator().setTier(Item.ItemTier.getByTier(tier))
                            ...
                            ItemStack itemStack = new ItemGenerator().setType(Item.ItemType.getRandomWeapon()).generateItem().getItem();
                            player.getInventory().addItem(itemStack);
                        }
                ...
                case "armor":
                    try {
                        if (args.length == 2) {
                            ...
                            ItemStack itemStack = new ItemGenerator().setType(Item.ItemType.getRandomArmor()).generateItem().getItem();
                            player.getInventory().addItem(itemStack);
                        }
                    } catch (NullPointerException ex) {
                ...
                case "customitem":
                    String name = args[1];
                    if (ItemGenerator.getNamedItem(name) != null) {
                ...
                case "pick":
                    ...
                    if (args.length == 2)
                        tier = Integer.parseInt(args[1]);
                    ItemStack itemStack = ItemManager.createPickaxe(tier);
                    break;
                case "rod":
                    int rodTier = 1;
                    if (args.length == 2)
                        rodTier = Integer.parseInt(args[1]);
                    ItemStack itemStackf = ItemManager.createFishingPole(rodTier);
                    player.getInventory().addItem(itemStackf);
                    break;
                case "resetbook":
